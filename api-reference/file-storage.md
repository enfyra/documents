...
Use `multipart/form-data` with a `file` field. Add `folder`, `title`, `description`, and `isPublic` in the form if needed.
...
`enfyra_file.isPublic` controls anonymous asset access through `/api/assets/{id}`. Use `isPublic: true` only for files that can be served without authentication. Column and relation visibility still uses `isPublished`; file public access does not.
...
Returns the file binary with appropriate `Content-Type`. Public files can be served anonymously when `enfyra_file.isPublic=true`; private files require the normal authenticated request path.
